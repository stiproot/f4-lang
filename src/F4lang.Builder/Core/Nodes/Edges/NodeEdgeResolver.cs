...
public class NodeEdgeResolver : INodeEdgeResolver
{
	...
	)
	{
		return nodeEdge.NodeEdgeType switch
		{
			...
			NodeEdgeTypes.Contextus => ResolveContextusNodeEdge(nodeEdge, msgs, workflowContext, cancellationToken),
	...
	{
		var edge = (nodeEdge as IMonariusNodeEdge)?.Edge ?? throw new InvalidOperationException();
	...
	private static async Task<IMsg[]> ResolveContextusNodeEdge(
		INodeEdge nodeEdge,
		IMsg[] msgs,
		IWorkflowContext? workflowContext,
		CancellationToken cancellationToken
	...
	{
		if (workflowContext is null) throw new ArgumentNullException(nameof(workflowContext));

		var key = (nodeEdge as IContextusNodeEdge)?.Key ?? throw new InvalidOperationException();

		var node = workflowContext.GetNode(key);

		if (node.NodeConfiguration.RequiresResult) node.AddArg(msgs);

		return await node.Resolve(cancellationToken);
	}
}