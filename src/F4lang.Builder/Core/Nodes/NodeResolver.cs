...
public class NodeResolver : INodeResolver
{
	...
	)
	{
		...
		{
			foreach (Func<IWorkflowContext, IMsg> f in node.NodeConfiguration.ContextArgs)
			{
				node.NodeConfiguration.Args.Add(f(node.NodeConfiguration.WorkflowContext));
			}
		}

		if (node.NodeConfiguration.ArgEnrichers.Any())
		{
			foreach (var (type, enricher) in node.NodeConfiguration.ArgEnrichers)
			{
				var arg = node.NodeConfiguration.Args.FirstOrDefault(x => x.ObjectData.GetType() == type || x.ObjectData.GetType().GetInterface(type.Name) is not null);
		...
		try
		{
			...
			if (node.NodeConfiguration.WorkflowContext is not null)
			{
				if (result is not null) node.NodeConfiguration.WorkflowContext.AddMsg(node.NodeConfiguration.Id, result);

				node.NodeConfiguration.WorkflowContext.AddNode(node.NodeConfiguration.Id, node);
			}
			...
			if (node.Controller is not null)
			{
				...
				if (bit is false) return [];
			}

			if (node.NodeEdge is null) return result.ToArray();

			return await this._EdgeResolver.Resolve(node.NodeEdge, result.ToArray(), node.NodeConfiguration.WorkflowContext, cancellationToken);
		}
		catch (Exception ex)
		{
