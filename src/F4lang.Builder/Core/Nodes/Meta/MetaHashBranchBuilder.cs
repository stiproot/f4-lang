...
public class MetaHashBranchBuilder : CoreBranchBuilder, IMetaBranchBuilder
{
	...
	{
		IFn fn = metaNode!.ServiceType.ToFn(this._FnFactory, metaNode.NodeConfiguration.NextParamName, metaNode.NodeConfiguration.PreProcess);
	...
	)
	{
		...
		Func<IArgs, IMsg?> decisionFn = StatDecisionFnFactory.Create(mn.NodeConfiguration.ControllerType, true, mn.NodeConfiguration.Key);
